...
let primeNumber = (n) => {
    /*let num = bigInt(2)*/
    let prime1 = [5021,5023,5039,5051,5059,5077,5081,5087,5099,5101,5107,5113,
        ...
        /*for (let i = 0; i < n; i++) {
            let a = prime1[Math.floor(Math.random() * (prime1.length))]
            console.log(a);
            num = num.multiply(a)
        }
        num.add(1)*/
        return bigInt(prime1[Math.floor(Math.random() * (prime1.length))])
}

let mod = (p, q) => {
    return bigInt(p.value * q.value)
}

let funEiler = (p, q) => {
    return bigInt(p.value - 1n).multiply(q.value - 1n)
}

let exp = (d, m) => {
    for (let e = bigInt(0); e < m; e++) {
        let ans = e
        ans = bigInt(ans).multiply(d).mod(m)
        if (ans.value === 1n) {
            return bigInt(e)
        }
    }
...
let textToNum = (text) => {
    ...
    for (let i = 0; i < text.length; i++) {
        l = text[i].charCodeAt(0).toString()
...
let numToText = (num) => {
    ...
    for (let i = 0; i < num.length / 3; i++) {
        l = String.fromCharCode(num.slice(i * 3, (i + 1) * 3))
        text += l
    }
    return text
}

let encryption = (e, n, text) => {
    let ans = bigInt(1)
    for (let i = bigInt(0); i < e; i++) {
        ans = bigInt(ans).multiply(text).mod(n)
    }
    return ans
}

let decryption = (d, n, text) => {
    let ans = bigInt(1)
    for (let i = bigInt(0); i < d; i++) ans = ans.multiply(text).mod(n)
    return ans
}
...
let numText = textToNum(text)
console.log("Text in numbers: " + numText);*/
let numText = bigInt(123456)
let text1 = encryption(e, n, numText)
console.log("Cipher: " + text1);
let text2 = decryption(d, n, text1)
console.log("Decryption: " + text2);
//console.log("Text: " + numToText(text2))
